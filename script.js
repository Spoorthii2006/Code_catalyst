...
function logout(){
  if(currentUser){
    alert(`👋 Bye ${currentUser.username}!`);
  }
  currentUser = null;
  updateUI();
}
...
function renderCart(){
  ...
  currentUser.cart.forEach(ci => {
    ...
    li.innerHTML = `<span>${escapeHtml(ci.product.title)} x ${ci.quantity}</span>
...
function checkout(){
  ...
  let total=0;
  currentUser.cart.forEach(i=> total += i.product.price * i.quantity);
